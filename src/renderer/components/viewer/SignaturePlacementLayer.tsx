...
/**
 * Transparent overlay that captures pointer/mouse drag to define
 * a signature placement rectangle on the PDF page.
 *
 * Modeled after FormFieldDrawLayer but dedicated to signature placement.
 */
export function SignaturePlacementLayer({
  ...
  const onCompleteRef = useRef(onPlacementComplete);
  ...
  useEffect(() => {
    onCompleteRef.current = onPlacementComplete;
  });
  ...
  function endDraw() {
    ...
    if (w >= 10 && h >= 10) {
      onCompleteRef.current(pageNumberRef.current, { x, y, width: w, height: h });
    }
  }
