...
interface RedactionDrawLayerProps {
  pageNumber: number;
  zoom: number;
  active: boolean;
  onRedactionDrawn: (
...
/**
 * Transparent overlay that handles click-drag rectangle drawing for redaction.
 * Always mounted in the DOM; pointer/mouse events are ignored when inactive.
 * Uses refs for all mutable state to avoid stale-closure issues with
 * useCallback dependencies and mount/unmount cycles.
 */
export function RedactionDrawLayer({
  pageNumber,
  zoom,
  active,
  onRedactionDrawn,
}: RedactionDrawLayerProps) {
  ...
  const onRedactionDrawnRef = useRef(onRedactionDrawn);
  ...
  useEffect(() => {
    onRedactionDrawnRef.current = onRedactionDrawn;
  });

  // Preview state (only used for rendering, not event logic)
  ...
  // ── Pointer / mouse event handlers (direct, no useCallback) ──
  ...
  function endDraw() {
    ...
    if (w >= 5 && h >= 5) {
      onRedactionDrawnRef.current(pageNumberRef.current, { x, y, width: w, height: h });
    }
  }

  // Attach/detach from DOM — uses addEventListener, not React props
  useEffect(() => {
    ...
    // MouseEvent fallback (belt-and-suspenders for Electron/automation)
    ...
    // Handlers use refs (stable across renders). Only attach once.
